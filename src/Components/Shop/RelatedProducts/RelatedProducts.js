...
const RelatedProducts = () => {
    ...
    return (
        <div>
            <h3 className='text-4xl font-bold text-center my-20'>Related Products</h3>
            <div className='grid md:grid-cols-4 ml-5'>
                {
                    bestSeller.map((seller, i) => <div
                        key={i}
                        className="card w-72 bg-base-100 mb-5">
                        <figure className='cursor-pointer'>
                            <img src={seller.imageUrl} alt="Shoes" className='border-2 rounded-xl border-gray-200	' />
                        </figure>
                        <div className="card-body text-left py-3 px-2 cursor-pointer">
                            ...
                            <h2 className="card-title hover:text-red-500 ease-out duration-500">{seller.name}</h2>
                            <p>${seller.price}</p>
                        </div>
